...
This package implements methods to provide perturbation for statistical tables. The method is based on the paper *Methodology for the Automatic Confidentialisation of Statistical Outputs from Remote Servers at the Australian Bureau of Statistics* (Thompson, Broadfoot, Elazar). Furthermore it is possible to perturb tables using a different approach developed by DESTATIS. The two approaches differ in the way record keys have to be specified, the format of the perturbation tables and the way, the actual lookup is done.
...
- different formats/algorithms for perturbation tables (`"abs"` and `"destatis"`) possible
- record-keys can be pre-generated for both variants
- computation of weighted and unweighted count tables
- computation of weighted and unweighted magnitude tables
- multidimensional tables with complex hierarchies in all dimensions are possible

## Todo
- ~~allow hierarchical input in  `perturbTable()`~~
- ~~check parametrisation of perturbation tables~~
- ~~usage of destatis input format for perturbation tables~~
...
## Summary
In case you have any suggestions or improvements, please feel free to file an issue at [**our issue tracker**](https://github.com/sdcTools/cellKey/issues) or contribute to the package by filing a pull request.