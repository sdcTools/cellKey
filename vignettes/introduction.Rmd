...
```{r}
tab1 <- perturbTable(inp=inp, dimList=dimList, weightVar="sampling_weight", numVars=NULL)
```
...
```{r}
tab2 <- perturbTable(inp=inp, dimList=dimList, 
...
Information on perturbations for counts can be extracted using `mod_counts()`. The method returns a `data.table` showing for each cell the row and the colum of the perturbation table that was used to find the perturbation value actually used. This value is also shown in the output.
```{r}
mod_counts(tab2) 
```

Information on perturbations for numerical variables can be extracted using `mod_numvars()`. For each variable that was perturbed, a `data.table` is returned showing the observations of the input variables that have been modified. For each observation, the parameters used in modifying (direction, magnitude and noise) are shown as well as the original values, the perturbation and the final modified value. These values were finally used to compute the tables itself.
```{r}
mod_numvars(tab2) 
```
