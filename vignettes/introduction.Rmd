...
### Loading testdata
The testdata set we are using in this example contains information on person-level along with sampling weights as well as some categorical and continously scaled variables. We also compute some binary variables for which we also want to create perturbed tables.
...
- `pTable`: a perturbation table containing perturbation values. Function `ck_create_pTable()` allows to create tables in the `abs` format given a flexible number of column (specified using parameter `pTableSize`). Such tables must have exactly 256 rows. An alternative way to specify perturbation tables was proposed by DESTATIS. An example perturbation in this format can be used by specifying `ck_create_pTable(type="destatis")`. In this case, parameters `bigN` and `smallN` are ignored because they are not needed in the algorithm to lookup perturbation values.
...
dim.sex2 <- ck_add_nodes(dim.sex2, reference_node="Total", node_labs=c("male","female"))
print(dim.sex2)
```
...
dim.age2 <- ck_add_nodes(dim.age2, reference_node="Total", node_labs=paste0("age_group",1:6))
print(dim.age2)
```
...
```{r}
tab1 <- perturbTable(inp=inp, dimList=dimList, countVars=c("cnt_males", "cnt_highincome"), 
...
```{r}
ck_freq_table(tab1) # whow which tables have been generated
...
The result obtained from `ck_cont_table` contains an attribute `modification` which holds a `data.table` containing all the perturbations for the variabe queried in `vname`. We can access this information as shown below:
...
tab3 <- perturbTable(inp=inp, dimList=dimList,  
  ...
  countVars=c("cnt_highincome"), 
  numVars=c("savings"), by="cnt_highincome")
```

We note that if the by_variable has been specified, it must also be listed in argument `countVars`. To show the table of variable `savings` we use again `ck_cont_table()`.

```{r}
p_savings <- ck_cont_table(tab3, vname="savings", meanBeforeSum=TRUE)
p_savings
```
...
Information on perturbations for counts can be extracted using `mod_counts()`. The method returns a `data.table` showing for each cell and tabulated variable the row and the colum of the perturbation table that was used to find the perturbation value actually used. This value is also shown in the output.
```{r}
