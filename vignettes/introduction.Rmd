...
```{r, echo = FALSE}
warning("find a suitable reference to the D4.2 deliverable")
```
...
- `parity`: should cells with an even/odd number of contributors perturbed using different ptables.

```{r}
# create a ptable using the ptable-pkg
...
p_nums1 <- ck_params_nums(
  ...
  top_k = 3,
  ptab = ptab,
  mult_params = mult_flex,
  mu_c = 2,
  same_key = FALSE)
```


## Todo: describe separation case

Such parameters can be attached to continuous variables using the `params_nums_set()`-method as shown below:

```{r}
tab$params_nums_set(v = c("income", "savings"), val = p_nums1)
```


Setting sensitive cells: 
Parameter `mu_c` in `ck_params_nums()` ... 

```{r, eval = FALSE}
tab$supp_p(v = "income", p = 85)
tab$supp_pq(v = "income", p = 85, q = 90)
tab$supp_nk(v = "income", n = 2, k = 90)
tab$supp_freq(v = "income", n = 5)
tab$supp_val(v = "income", n = 1000)
```
