...
test_that("onlyzero equals total", {
  expect_identical(dim(r0), dim(r2))
  expect_identical(sum(r2[, UWC_cnt_onlyzero]), 0)
  expect_identical(sum(r2[, WC_cnt_onlyzero]), 0)
  expect_identical(sum(r2[, pUWC_cnt_onlyzero]), 0)
  expect_identical(sum(r2[, pWC_cnt_onlyzero]), 0)
  expect_identical(sum(r2[, WCavg_cnt_onlyzero]), 0)
  expect_identical(as.integer(sum(r2[, row_indices])), nrow(r2))
  expect_identical(sum(r2[, col_indices]), 0)
  expect_identical(sum(r2[, cellKey]), 0)
})
