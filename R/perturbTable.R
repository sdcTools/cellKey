...
perturbTable <- function(
  ...
  count_modifications <- cbind(
    tab[, c(names(dimList)), with = FALSE],
    count_modifications
  )
  ...
  if (!is.null(numVars)) {
    ...
    res1 <- tab[, lapply(.SD, function(x) x * get(by_wgt)), .SDcols = nvOrig]
    tab[, c(cn1) := res1]

    res2 <- tab[, lapply(.SD, function(x) x * get(by_wgt)), .SDcols = nvPert]
    tab[, c(cn2) := res2]
