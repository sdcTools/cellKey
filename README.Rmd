...
For the sake of simplification, the differentiation between the *"abs"* and *"destatis"* format was removed in version `>= 0.17`. Internally, only the "destatis" format is used.
...
This features was removed in versions `0.17.x` and added again in versions `>= 0.18.0`. For examples please look at `?cellkey_pkg` or at the [**vignette**](https://sdctools.github.io/cellKey/articles/introduction.html).
